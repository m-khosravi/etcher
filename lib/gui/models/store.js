...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SELECT_IMAGE: {
      ...
      const MINIMUM_DRIVE_SIZE = 0;

      if (!_.isInteger(action.data.size.original) || action.data.size.original < MINIMUM_DRIVE_SIZE) {
        throw errors.createError(`Invalid original image size: ${action.data.size.original}`);
      }

      if (!_.isInteger(action.data.size.final.value) || action.data.size.final.value < MINIMUM_DRIVE_SIZE) {
